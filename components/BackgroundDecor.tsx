...
function getIllustrated(): boolean {
  if (typeof document === "undefined") return false;
  return document.documentElement.classList.contains("illustrated");
}
...
export default function BackgroundDecor() {
  ...
  const baseSize = illustrated ? "448px" : "320px";
  const bookSize = illustrated ? "504px" : "360px";
  const netSize = illustrated ? "392px" : "280px";
  const penSize = illustrated ? "336px" : "240px";

  const brainColor = illustrated ? "#818cf8" : "var(--accent)";
  const bookColor = illustrated ? "#34d399" : "var(--accent)";
  const netColor = illustrated ? "#fb923c" : "var(--accent)";
  const penColor = illustrated ? "#f472b6" : "var(--accent)";

  const brainOpacity = illustrated ? 0.18 : 0.04;
  const bookOpacity = illustrated ? 0.18 : 0.05;
  const netOpacity = illustrated ? 0.18 : 0.04;
  const penOpacity = illustrated ? 0.18 : 0.05;
    ...
    <>
      ...
      <svg
        style={{
          ...
          transition: "opacity 0.4s ease, color 0.4s ease, width 0.4s ease, height 0.4s ease",
        }}
        viewBox="0 0 100 100"
        fill="currentColor"
        aria-hidden="true"
